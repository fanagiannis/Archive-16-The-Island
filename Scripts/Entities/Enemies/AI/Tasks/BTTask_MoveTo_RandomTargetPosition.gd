...
func tick(actor: Node, blackboard) -> int:
	var target_pos = blackboard.get_value("TargetPosition")
	var speed = blackboard.get_value("AgentSpeed", 5.0)
	...
	if nav_agent.is_navigation_finished() or dist_to_target < 1.2:
