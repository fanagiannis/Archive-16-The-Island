...
public partial class Mouse : Node3D
{
	...
	public override void _UnhandledInput(InputEvent @event)
	{
		...
		if (UseController)
		{
			if (@event is InputEventJoypadMotion eventJoypadMotion && controllerEnabled)
			{
		...
		{
			if (@event is InputEventMouseMotion eventMouseMotion && controllerEnabled)
			{
				// Horizontal movement of head
				...
				currentCameraRotation.X = Mathf.Clamp(currentCameraRotation.X, Mathf.DegToRad(-89f), Mathf.DegToRad(89f));
	...
	public void EnableController()=>controllerEnabled = true;
	public void DisableController()=>controllerEnabled = false;
